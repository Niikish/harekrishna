...
export default function Navbar() {
  ...
  return (
    ...
    >
        ...
        <div className="flex justify-between items-center">
          ...
          >
            <div className="relative">
              <Image
                ...
                height={100}
                priority
                className={`object-contain transition-all duration-500 group-hover:scale-110 group-hover:rotate-3 drop-shadow-md group-hover:drop-shadow-2xl ${isScrolled ? "w-14 h-14 sm:w-16 sm:h-16" : "w-16 h-16 sm:w-20 sm:h-20"
                  }`}
              />
              <div className={`absolute -bottom-1 left-1/2 transform -translate-x-1/2 h-0.5 bg-red-600 transition-all duration-300 ${isScrolled ? "w-10" : "w-12 sm:w-14"
                } group-hover:w-full opacity-0 group-hover:opacity-100`}></div>
            </div>

            <div className={`flex flex-col ml-2 transition-all duration-300 ${isScrolled ? "pt-2" : "pt-4"
              }`}>
              <h1 className={`text-red-600 font-medium tracking-wider transition-all duration-300 ${isScrolled ? "text-base sm:text-lg" : "text-lg sm:text-xl"
                }`}>
                Hare Krishna Projects
              </h1>
              {/* <h2 className={`text-red-600 font-medium tracking-wider transition-all duration-300 ${isScrolled ? "text-base sm:text-lg" : "text-lg sm:text-xl"
                }`}>
                & Developers
              </h2> */}
              <p className={`text-gray-600 tracking-wide font-light transition-all duration-300 ${isScrolled ? "text-xs mt-0.5" : "text-sm mt-1"
                }`}>
          ...
          <nav className="hidden lg:flex items-center space-x-8">
            {menuItems.map((item) => (
              ...
              >
                <span className={`${isScrolled ? "text-green-800" : "text-white"
                  } group-hover:text-red-600 transition-colors duration-300`}>
                  {item.title}
                </span>
                <span className={`absolute -bottom-1 left-0 w-full h-0.5 transition-all duration-300 transform origin-left ${activeSection === item.id
            ...
            <a
              ...
              className={`relative overflow-hidden py-2 px-4 rounded-full transition-all duration-300 ${isScrolled
          ...
          >
            <div className="relative w-6 h-5">
              <span className={`absolute top-0 left-0 w-6 h-0.5 rounded-full transition-all duration-300 ease-in-out ${isScrolled || isMobileMenuOpen ? "bg-green-800" : "bg-white"
                } ${isMobileMenuOpen ? "transform rotate-45 translate-y-2" : "group-hover:w-5"}`}></span>

              <span className={`absolute top-1/2 left-0 w-4 h-0.5 rounded-full -translate-y-1/2 transition-all duration-300 ease-in-out ${isScrolled || isMobileMenuOpen ? "bg-green-800" : "bg-white"
                } ${isMobileMenuOpen ? "opacity-0 w-6" : "group-hover:w-6"}`}></span>

              <span className={`absolute bottom-0 left-0 w-6 h-0.5 rounded-full transition-all duration-300 ease-in-out ${isScrolled || isMobileMenuOpen ? "bg-green-800" : "bg-white"
      ...
      <div
        className={`fixed inset-0 bg-gradient-to-b from-white to-green-50 z-40 transform transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? "translate-x-0" : "translate-x-full"
            ...
            {menuItems.map((item) => (
              <div key={item.id} className="py-3 border-b border-green-100">
                <Link
                  href={`#${item.id}`}
                  onClick={(e) => handleNavigation(e, item.id)}
                  className={`block font-medium text-green-800 hover:text-green-600 transition-colors duration-200 text-lg flex items-center ${activeSection === item.id ? "font-semibold" : ""
                    }`}
                >
